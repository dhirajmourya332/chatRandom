...
async function setup_node_instance_zmq_socket_socketinterconnection() {
  return new Promise(async (resolve, reject) => {
    ...
    async function connectToCommonSocket() {
      return new Promise((resolve, reject) => {
        try {
          ...
          console.log("failed!");
          if (
            new Error(error).message.match(/Address already in use/gi).length
          ) {
            setTimeout(() => {
              resolve(connectToCommonSocket());
            }, Math.random() * 500);
          } else reject(error);
        }
      });
...
(async function () {
  try {
    await setup_node_instance_zmq_socket_socketinterconnection();
